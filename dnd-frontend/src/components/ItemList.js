...
import api from "../services/api";              // axios yerine ortak servis
import "./ItemList.css";
...
export default function ItemList() {
  ...
  useEffect(() => {
    // → aynı router mantığı: /api/items/items/
    api
      .get("items/items/")
      .then((res) => {
        // pagination varsa results, yoksa doğrudan liste
  ...
  if (loading) return <div className="item-list-container"><p>Loading items...</p></div>;
        ...
        {items.map((item) => {
          ...
          return (
            <div key={id} className="item-card">
              ...
              {damageExpr && <p><strong>Damage:</strong> {damageExpr}</p>}
              {ac_bonus > 0 && <p><strong>AC Bonus:</strong> +{ac_bonus}</p>}
